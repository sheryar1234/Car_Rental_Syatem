...
const RenterSignup = () => {
  ...
  const handleRenterSignup = async (e) => {
    e.preventDefault();

    try {
      const response = await axios.post('http://localhost:5000/api/auth/renter-signup', { email, password });
  ...
  return (
    <div className="relative h-full min-h-screen font-[sans-serif]">
      ...
      <div className="relative z-10 flex justify-end items-center h-full p-11 ">
        <div>
          <form onSubmit={handleRenterSignup} className="bg-opacity-90 bg-black rounded-2xl p-6 pl-14 pr-14 mt-12">
            <div className="mb-12">
