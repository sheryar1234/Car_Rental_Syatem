...
const toggleCustomerStatus = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: 'User status updated successfully', isAllowed: user.isAllowed });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Server error' });
  }
...
module.exports = { customerSignup, customerLogin, getAllCustomers, deleteCustomer, toggleCustomerStatus };